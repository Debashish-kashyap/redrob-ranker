...
This repo does NOT include `candidates.jsonl` (487MB, excluded via .gitignore
to keep the repo lightweight). Place the official hackathon dataset at:

```
data/candidates.jsonl
```

before running precompute.py or rank.py below. Also excluded for the same
reason: `embeddings.npy`, `candidate_ids.json`, and the downloaded model
under `models/` — all three are regenerated by the one-time setup commands
below and do not need to be committed.
